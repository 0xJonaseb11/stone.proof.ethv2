...
const deployMineralSystem: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log("Deploying MineralLicenseManager...");
  const licenseManager = await deploy("MineralLicenseManager", {
    from: deployer,
    args: [],
    log: true,
    autoMine: true,
  ...
  await logGasUsed("MineralLicenseManager", licenseManager);
